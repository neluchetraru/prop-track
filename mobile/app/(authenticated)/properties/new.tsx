...
import TenantsStep from "./steps/TenantsStep";
...
// PropertyFormData is now imported from @/api/properties
    ...
    mutationFn: async (data: PropertyFormData) => {
      ...
      const uploadedDocuments = await Promise.all(
        (data.documents || []).map(async (doc) => {
          ...
          return { url: "temporary-url", type: doc.type }; // Replace with actual upload
        })
      );
  ...
  return (
    <>
      ...
      <View className="flex-row border-t border-gray-200 p-4 gap-4 justify-between bg-white">
        <TouchableOpacity
          className={`flex-1 h-12 rounded-lg justify-center items-center ${currentStep === 0 ? "bg-gray-100" : "bg-blue-50"
            }`}
        ...
        >
          <View className="flex-row items-center gap-2">
            ...
            />
            <Text
              className={`font-bold ${currentStep === 0 ? "text-gray-400" : "text-blue-700"
                }`}
            >
              Back
            </Text>
